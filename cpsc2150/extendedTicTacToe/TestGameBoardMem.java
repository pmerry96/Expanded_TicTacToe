...
public class TestGameBoardMem
{
    private IGameBoard gb(int boardrow, int boardcol, int wincond) {
        return new GameBoardMem(boardrow, boardcol, wincond);
    }
    ...
    public void testConstructor_maxSize()
    {
        ...
        //add in a test for the tostring representation - Ask kev if its acceptable to make a "shorthand" means of this
            // IE remove formatting, and just print the chars sequentially.
    ...
    public void TestWhatsAtPos_PlayerInBottomRight()
    {
        ...
        IGameBoard t = gb(3,3,3);
        t.placeMarker(new BoardPosition(2,2), 'X');
        assertEquals('X', t.whatsAtPos(new BoardPosition(2, 2)));
    }
    ...
    public void TestisPlayerAtPos_NoCharAtPos()
    {
        ...
        //This is an interesting case in which I do expect the behavior of these methods to be different
        //given their underlying data type. And that is because they are PRIMARY METHODS, which must rely
        //on private data to perform their actions. In GameBoardMem, isplayeratpos relies on the player existing in the
        //playerlist. We do not list the space character as a player because
        //1 - it is not a player.
        //2 - the purpose of this implementation is memory efficiency. If we stored every empty location under the space
        //  character "psuedoplayer", we negate the memory efficiency savings.
        //I will replace this check with a "whatsatpos", and use assertequals because that will mirror the purpose of the
        //original check as well as preserve the integrity of this test case
             ...
             */
        assertEquals(t.whatsAtPos(new BoardPosition(1, 1)), ' ');
            //the reason I use this is because it essentially mirrors the check inside isplayeratpos in gameboard
            // Its roundabout but it verifies things function as I expect them *IN THIS SPECIFIC INSTANCE AND USE*
    }
